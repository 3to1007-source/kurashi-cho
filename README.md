...
- **家計**: 「使えるお金」(家計の収入とは別枠、ユーザーごとに開始日・締日・月額を設定できるお小遣い制の予算。締日までの未来日付の支出も先取りで差し引いて「今月あと◯◯円」を表示)、カテゴリ予算(食費・交際費など分類ごとの月の予算。誰の記録でも合算して残額を表示)、月次の収入・支出・差引サマリー、支出カテゴリ別の横棒グラフ、記録フォーム、当月の記録一覧
